...
| ---------------------------- | -------------------------------------------------------------------------------------- |
| `Gender`                     | `MALE`, `FEMALE`                                                                       |
| `NotificationTopic`          | `PRAYER_TIME`, `MARK_WINDOW_CLOSING`, `STREAK_AT_RISK`, `NEW_FOLLOWER`                 |
| `Madhab`                     | `SHAFI`, `HANAFI`                                                                      |
...
| `NotificationDeliveryStatus` | `SENT`, `FAILED`, `SUBSCRIPTION_GONE`, `NO_DEVICE`                                     |
...
| `20260829193838_drop_unused_push_fasting_mastery` | drops `fasting_days`, `push_subscriptions`, `notification_preferences`, `notification_deliveries` and `question_mastery` (all empty) with their FKs, and drops the `FastingStatus` / `NotificationTopic` / `NotificationDeliveryStatus` enums — three features that were scaffolded in the schema but never built                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
`push_subscriptions`, `notification_preferences` and `notification_deliveries` were scaffolded by
`20260822115649_audit_fixes_and_features` and sat writer-less until the feature was built in
`20260829200000_web_push_notifications` (which also narrowed `NotificationTopic`, see below). They
are now live — `src/notifications/` owns all three.

| Table                      | Lifecycle                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `push_subscriptions`       | One row per browser. `endpoint` is the identity (`@unique`), so re-subscribing the same browser upserts instead of duplicating. Deleted on 404/410 from the push service, or after `PUSH_MAX_FAILURES` consecutive failures.                                                                                                                                                                      |
| `notification_preferences` | KVKK consent record, one row per `(user, topic)`. `enabled` defaults to **false**; a missing row means disabled. **Rows are never deleted on opt-out** — `optedInAt` / `optedOutAt` are the audit trail.                                                                                                                                                                                          |
| `notification_deliveries`  | Send log **and** in-app feed. `(userId, dedupeKey)` is unique and is the idempotency mechanism, not just a report: `NotificationDispatchService` inserts the row _before_ sending. `title` / `body` / `url` carry the rendered notification; `readAt` is the bell's unread state. Pruned after `NOTIFICATION_DELIVERY_RETENTION_DAYS` (30) by a daily cron, which also bounds the feed's history. |

A row is kept even when the push cannot be delivered — `NO_DEVICE` (no subscription), `FAILED`, or
`SUBSCRIPTION_GONE` all still appear in the in-app feed, because a notification the user consented
to should reach them when they next open the app. Only a **missing consent** deletes the row
(`dispatch()`'s release branch), and that row was never a notification in the first place. Added by
`20260829233000_notification_feed`.

`p256dh` / `auth` are RFC 8291 encryption keys. They are security material, not the user's own data,
and are excluded from `GET /users/me/export` — the same reasoning as the password hash.

### Removed feature: writer-less tables (Ramazan modu, quiz mastery)

The same migration created `fasting_days` (Ramazan modu) and `question_mastery` (quiz spaced
repetition) for features that were never built. Nothing in `src/` ever wrote to them; the only
reader was the KVKK export, so those sections were always empty.

`20260829200000_web_push_notifications` dropped both (0 rows each, verified before the drop) plus
the `FastingStatus` enum, and removed `RAMADAN_SUHOOR` / `RAMADAN_IFTAR` from `NotificationTopic`
while adding `NEW_FOLLOWER`. `GET /users/me/export` lost its `fasting` section and the
`quizzes.mastery` key; the format version went to **2**.

Two things a future agent must not misread:

1. Rebuilding Ramazan modu means writing a **new** migration for what that feature actually needs —
   do not resurrect the old shape on the assumption it was designed against real requirements. It
   never ran.
2. `fasting` in `/worship` is a _computed_ daily progress (`FastingProgressService`) and never had
   anything to do with `fasting_days`. It is unaffected by the drop and still works.
